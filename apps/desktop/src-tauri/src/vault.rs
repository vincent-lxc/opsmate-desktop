//! Local Stronghold credential vault (Task D3, security repair).
//!
//! WebView IPC never accepts tenant_id/user_id/subject/snapshot_path.
//! Stronghold namespace is **tenant_id + Logto subject + credential_id** only
//! (never username). Bound from the authenticated native principal.
...
// ─── Safe IPC DTOs (no tenant/user/path from WebView) ────────────────────────

/// Design: only `unlocked` + `locked_reason` — never expose active tenant/user IDs.
...
// Locked is small; Unlocked holds Stronghold + state. Boxing would touch every
// critical path for little gain — allow the size asymmetry intentionally.
...
impl VaultService {
    ...
    pub fn unlock_with_password(
        ...
        let principal = auth.native_principal().ok_or(VaultError::Unauthenticated)?;
    ...
    /// Internal: if authenticated principal differs from unlocked vault principal, lock.
    pub fn observe_principal(&self, auth: &AuthStore) -> Result<(), VaultError> {
        let action = {
            let guard = self.lock_inner()?;
            if let VaultInner::Unlocked(u) = &*guard {
                match auth.native_principal() {
                    None => Some("logout"),
                    Some(p) if p != u.principal => Some("principal_changed"),
                    _ => None,
                }
            } else {
                None
    ...
    pub fn delete_local(&self, auth: &AuthStore, credential_id: &str) -> Result<(), VaultError> {
        ...
        let _op = self.enter_op()?;
        // Scoped close only after claim; principal-bound (tenant isolation).
    ...
    pub fn import_begin_native(
        ...
        let _ = auth.native_principal().ok_or(VaultError::Unauthenticated)?;
        ...
        self.import_pem(auth, &req.credential_id, pem, passphrase)
    }
    ...
    pub fn lease_for_ssh(
        ...
        validate_id(credential_id)?;
        if !auth.session_binding_current(expected_principal, expected_epoch) {
            return Err(VaultError::Unauthenticated);
        }
        self.observe_principal(auth)?;
        let _op = self.enter_op()?;
        ...
        let raw = {
            ...
            if u.principal != *expected_principal {
                return Err(VaultError::Unauthenticated);
            }
            // Re-check binding while we hold vault state (epoch cannot be checked under
            // AuthStore without releasing; revalidate after load as well).
            u.last_activity = Instant::now();
            let client = u
                .stronghold
                .get_client(CLIENT_PATH)
    ...
    /// Test-only: claim OPERATING (RAII) for race tests.
    #[cfg(test)]
    pub fn test_claim_operating(&self) -> Result<OpGuard<'_>, VaultError> {
        self.claim_operating()
    }
}
...
pub(crate) struct OpGuard<'a>(pub(crate) &'a VaultService);
